...
use crate::server::utils::scoreboard::scoreboard_line::ScoreboardLine;
...
impl Scoreboard {
    ...
    pub fn clear(&self, packets: &mut Vec<ClientBoundPacket>) {
    ...
    pub fn handle_add_and_remove(&mut self) {
        if !self.to_remove.is_empty() {
            self.lines.retain(|key, _| !self.to_remove.contains(key));
            self.to_remove.clear();
        }

        // this could be faster if we rebuild the map instead of shifting elements... not sure if it matters.
    ...
    pub fn push_lines_to(&mut self, packets: &mut Vec<ClientBoundPacket>, dirty: bool) {
        let len = self.lines.len();
    ...
    pub fn get_packets(&mut self) -> Vec<ClientBoundPacket> {
        ...
        if dirty {
            self.clear(&mut packets);
        }

        self.handle_add_and_remove();

        self.push_lines_to(&mut packets, dirty);
